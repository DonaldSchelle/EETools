...
|iReturnValue||Return Value<br/>0 = Return an array of values.  Highlight desired number of cells, enter  formula, and press **CTRL**+**SHIFT**+**ENTER**<br/>1 = Return a single value for R~PRIMARY~<br/>2 = Return a single value for RSECONDARY<br/>3 = Return a single value for RTERTIARY<br/>4 = Return a single value error between the target ratio and the calculated E-series ratio, calculated according to iCalcType|
